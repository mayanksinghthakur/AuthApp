...
import com.jwtDemo.auth.io.AuthRequest;
import com.jwtDemo.auth.io.AuthResponse;
import com.jwtDemo.auth.io.ProfileResponse;
import com.jwtDemo.auth.io.ResetPasswordRequest;
import com.jwtDemo.auth.service.AppUserDetailsService;
import com.jwtDemo.auth.service.ProfileService;
import com.jwtDemo.auth.util.JwtUtil; // Add this import
...
public class AuthController {
    ...
    private final JwtUtil jwtUtil; // Add JWT utility
    ...
    return ResponseEntity.ok().header(HttpHeaders.SET_COOKIE,cookie.toString()).body(new AuthResponse(request.getEmail(),jwtToken));
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(error); // Fixed: Added return

        } catch (Exception exc) {
            Map<String, Object> error = new HashMap<>();
            error.put("error", true);
            error.put("message", "Authentication failed"); // Fixed grammar
    ...
    //code for autheneticationod the email and password
    private void authenticate(String email, String password) {
        authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(email,password));
    }

    //endpoint for react to check weather logged in user exist or not
    ...
    //using this api our email will receive the otp
    ...
    //using map collection to get otp
    ...
    public ResponseEntity<?>logout(HttpServletResponse response){
        ResponseCookie cookie=ResponseCookie.from("jwt","value").httpOnly(true).secure(false).path("/").maxAge(0).sameSite("strict").build();
